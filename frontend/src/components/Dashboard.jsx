...
// Define la estructura de datos inicial esperada si el hook es nulo
...
  // 🛑 CORRECCIÓN CLAVE: Inicializar 'datos' con la estructura vacía 
  // si el hook aún no ha cargado los datos (dashboardData puede ser null o undefined).
  const datos = dashboardData || EMPTY_DASHBOARD_DATA; 

  const dataGrafica = [
    // Usamos encadenamiento opcional para la gráfica, aunque la inicialización ya lo protege.
    { name: 'Almacén', valor: datos.porArea?.ALMACEN || 0, color: '#3498db' },
    { name: 'Cocina', valor: datos.porArea?.COCINA || 0, color: '#e74c3c' },
    { name: 'Ensaladas', valor: datos.porArea?.ENSALADA || 0, color: '#2ecc71' },
    { name: 'Isla', valor: datos.porArea?.ISLA || 0, color: '#f1c40f' },
  ];
...
    // Usamos encadenamiento opcional en caso de que 'ingrediente' o 'nombre' sean nulos
    item.ingrediente?.nombre?.toLowerCase().includes(filtro.toLowerCase())
  );
  
  // Manejo de estado de carga y error antes del renderizado del contenido
...
        </div>
        {/* Nota: El gráfico ya no se renderizará si no hay datos en dataGrafica */}
      </div>
